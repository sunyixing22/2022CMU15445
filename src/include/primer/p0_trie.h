...
class Trie {
 ...
 public:
  ...
  auto Insert(const std::string &key, T value) -> bool {
    ...
    c--;
    auto end_note = pre_child->get()->GetChildNode(*c);
    if (end_note != nullptr && end_note->get()->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }
    if (end_note != nullptr) {
      auto new_node = new TrieNodeWithValue(std::move(**end_note), value);
      end_note->reset(new_node);
      latch_.WUnlock();
      return true;
    }
  ...
  auto Remove(const std::string &key) -> bool {
    ...
    while (!s.empty()) {
      ...
      auto falg = (*node)->GetChildNode(key);
      if (falg != nullptr && (*falg)->HasChildren()) {
        continue;
      }
